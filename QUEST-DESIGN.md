...
- **Jormungandr — encuentro de avistamiento desde la costa (añadido 2026-09-22)**: segundo encuentro, **antes** del de arriba (que ocurre bajo el agua al coger el material 1 y **no se toca**): al llegar a la zona, el jugador ve a la criatura cruzar el mar de lejos. Misma estructura, con referencias propias que no comparten nada con la escena submarina. Las posiciones de A/B/trigger siguen en ajuste por prueba; la fuente de verdad es el `.esp`.
  - Criatura: `CAP_ThorMjolnir_Jorumngandr_Reference02` (REFR `030FBF4B`; el typo "Jorumngandr" está en el EditorID). Base `CAP_ThorMjolnir_Activator_Jormungandr`, Persistent + Initially Disabled, escala 8, `CAP_ThorMjolnir_JormunganderAlertEnd` en la propia referencia. Su posición es el punto A, distinto de la de `Reference01` (escena submarina). Full LOD marcado por el usuario (el flag `0x10000` apareció en el `.esp` al marcarlo); resultado sin probar en el juego.
  - Punto B: `CAP_ThorMjolnir_XmarkerPointB` (REFR `030FBF46`, base `00000034` de Skyrim.esm, la misma que el B de la otra escena).
  - Trigger: `CAP_ThorMjolnir_Trigger_JormunganderFirstEncounter` (REFR `030FBF4A`, base `00000F55` de Skyrim.esm, celda `(-22, 35)`, primitiva box ≈ 3843 × 12635 × 450 con el eje largo este-oeste). Lleva `CAP_JorumnandrAlertTrigger` en la propia referencia: `CreatureRef` = `Reference02`, `TargetMarker` = `XmarkerPointB`, `PlayerRef` = jugador, `ApproachSound` = el mismo Sound Marker que la otra escena. Verificado leyendo el `.esp` (2026-09-22): el trigger de la escena submarina sigue apuntando a `Reference01` y a su propio B, sin cruces.
  - **Cambio de script 2026-09-22**: `CAP_JorumnandrAlertTrigger` gana `Float Property TravelSpeed = 400.0 Auto` (antes `TranslateToRef(TargetMarker, 400.0)` fijo). El trigger de la escena submarina no la rellena y conserva 400; el nuevo la rellena en la CK. ✅ compila al scratchpad (0 errores / 0 avisos; el bytecode muestra valor inicial 400.0 y `TranslateToRef` la usa). ⚠️ Pendiente: recompilar en la CK, rellenar la property en el trigger nuevo y probar. Un valor puesto en la CK no se aplica a instancias del script ya guardadas en un save (comportamiento conocido de Papyrus, no verificado aquí): probar con un save anterior a la primera carga de esa celda.
  - **Limitación conocida — celdas cargadas**: el perfil (`skyrim.ini`) tiene `uGridsToLoad=5` (±2 celdas alrededor del jugador, celdas de 4096 u) y una celda sin cargar no dibuja la criatura, así que si el jugador está lejos aparece "de la nada" al acercarse. Al ajustar A/B/trigger, comprobar que el recorrido cae dentro de esas ~2 celdas respecto a donde el jugador dispara el trigger.
  - **Border Region**: leído el polígono `BorderRegionSkyrim` (REGN `000C5859`, Skyrim.esm; `bBorderRegionsEnabled=1`). Su interior es la zona jugable (el centro de Tamriel cae dentro). A, B y el trigger (versión del 2026-09-22 00:37) quedaron dentro, igual que `Reference01`, que se mueve bien; no se pudo saber de qué lado estaba el fallo original. Solo se leyó `Skyrim.esm`, no el resto del load order.
